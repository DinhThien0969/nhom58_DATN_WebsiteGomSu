$(document).ready(function() {
    ...
    function ajaxPostLapTop() {
    	// PREPATEE DATA
    	 ...
    	 $.ajax({
 			...
 			type : "POST",
 			contentType : "application/json",
 			url : "http://localhost:8080/potteryshop/api/san-pham/save",
    ...
    $(document).on("click",".btnXoaSanPham", function() {
		...
		if(confirmation){
		  $.ajax({
			  async:false,
			  type : "DELETE",
			  url : "http://localhost:8080/potteryshop/api/san-pham/delete/" + sanPhamId,
    ...
    $(document).on("click",".btnChiTiet", function() {
    	...
    	var href = "http://localhost:8080/potteryshop/api/san-pham/"+sanPhamId;
		$.get(href, function(sanPham) {
			$('.hinhAnh').attr("src", "/potteryshop/img/"+sanPham.id+".png");
			...
			if(checkTenDanhMuc != -1){
			  $('.mauSac').html("<span style='font-weight: bold'>Màu sắc: </span>"+ sanPham.mauSac);
			  $('.kichThuoc').html("<span style='font-weight: bold'>Kích thước: </span>"+ sanPham.kichThuoc);
			...
			$('.soLuong').html("<span style='font-weight: bold'>Số lượng trong kho: </span>"+ sanPham.soLuong);
