$(document).ready(function() {
	...
	function ajaxGet(page){		
		...
		$.ajax({
			...
			url: "http://localhost:8080/potteryshop/api/don-hang/all" + '?page=' + page,
...
//		event.preventDefault();
	...
	$(document).on('click', '.btnChiTiet', function (event){
		...
		var donHangId = $(this).parent().prev().children().val();	
//		console.log(donHangId);
		var href = "http://localhost:8080/potteryshop/api/don-hang/"+donHangId;
...
donHangRow += ' &nbsp;<button class="btn btn-warning btnCapNhat" >Duyệt hoàn thành</button> </td>';
	...
	function ajaxPostXacNhanHoanThanh() { 
    	 $.ajax({
 			...
 			type : "POST",
 			contentType : "application/json",
 			url : "http://localhost:8080/potteryshop/api/don-hang/update?donHangId="+$("#idDonHangXacNhan").val()+"&ghiChu="+$("#ghiChuAdmin").val(),
 			enctype: 'multipart/form-data',
			success : function(response) {
