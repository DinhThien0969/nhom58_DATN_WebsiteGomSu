$(document).ready(function() {
	...
	function ajaxGet(page){		
		...
		$.ajax({
			...
			url: "http://localhost:8080/potteryshop/api/tai-khoan/all" + '?page=' + page,
			success: function(result){
				$.each(result.content, function(i, taiKhoan){
					var taiKhoanRow = '<tr>' +
					  ...
					  taiKhoanRow += '<td width="0%">'+'<input type="hidden" id="idTaiKhoan" value=' + taiKhoan.id + '>'+ '</td>';
					  if(taiKhoan.isBlocked) taiKhoanRow += '<td></td>';
					  else taiKhoanRow+= '<td></td>'
					  ;
...
function ajaxPostTaiKhoan() {    	
    	...
    	console.log(data);
    	 ...
    	 $.ajax({
 			...
 			type : "POST",
 			contentType : "application/json",
 			url : "http://localhost:8080/potteryshop/api/tai-khoan/save",
    ...
    $(document).on("click",".btnXoa", function() {
		...
		if(confirmation){
		  $.ajax({
			  type : "DELETE",
			  url : "http://localhost:8080/potteryshop/api/tai-khoan/delete/" + taiKhoanId,
