...
<%-- <c:if test="${fn:length(cartNew)>0 && fn:length(cartOld)>0 }">
<td colspan="7">SẢN PHẨM CŨ</td>
</c:if> --%>
...
<tr class="cart_line" id="item${sanpham.id}">
		...
		<td class="covertPriceProduct">${sanpham.donGia}</td>
		<td>
			<input class="input_num_cart" type="number" value="${quanity[sanpham.id]}" min="1" onChange="changeQuanityNew(${sanpham.id},this.value,${sanpham.donGia})">
		</td>
		<td><b><span class="total" id="item${sanpham.id}_total" name="total_price">${sanpham.donGia*quanity[sanpham.id] } </span> VND</b></td>
		<td class="cart-img">
