		...
		<div class="row">
			...
			<div class="col-md-4">
					...
					<p>
						...
						<select id="mylist" onchange="searchByStatus()"
							class='form-control'>
							<option value="">Tất cả</option>
							<option value="Vui lòng đợi nhân viên xác nhận">Chờ xác
								nhận</option>
					...
					<p>
					<h3>Tìm kiếm theo mã đơn hàng</h3>
					</p>
					<input type="text" class='form-control' id="myInput"
						onkeyup="searchById()" placeholder="Nhập id đơn hàng">
					<p>
					<h3>Tìm kiếm theo ngày mua hàng</h3>
					</p>
					...
					<c:forEach var="donHang" items="${list}" varStatus="loop">
						...
						<tr style="text-align: center;">
							...
							<td><c:forEach var="chiTiet"
									...
									<p>
										<a href='<c:url value="/sp?id=${chiTiet.sanPham.id}" />'>${chiTiet.sanPham.tenSanPham}
										</a> <br>
									</p>
									<c:set var="donGia" value=" ${chiTiet.sanPham.donGia}" />
							...
							<td><c:choose>
									<c:when
										test='${(donHang.trangThaiDonHang == "Đang chờ xác nhận khách mua")}'>
							...
							<td><c:if
									test='${(donHang.trangThaiDonHang == "Đang giao")}'>
									<input class="donHangId hidden" type="text"
										value="${donHang.id}" />
									<button class="btn btn-primary btnDaNhanHang">Đã nhận
										hàng</button>
								</c:if> <!-- <hr> --> <input class="baoCaoId hidden" type="text"
								value="${donHang.id}" /> <!-- <button type="button" class="btn btn-danger btnSuCo" >Báo cáo sự cố</button>    -->
							</td>
						</tr>
					</c:forEach>
				</table>
